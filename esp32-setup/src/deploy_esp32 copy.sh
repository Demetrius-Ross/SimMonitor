...
# Function to ask user for COM port on Windows (no PowerShell)
detect_com_port() {
    echo "🔍 Detecting ESP32 device on Windows..."
    echo "💡 Tip: Open Device Manager → 'Ports (COM & LPT)' to see which COM port your ESP32 is on."
    echo

    while true; do
        read -p "👉 Enter the COM port to use (e.g., COM6 or COM15), or 'q' to quit: " ESP_DEVICE

        if [ "$ESP_DEVICE" = "q" ] || [ "$ESP_DEVICE" = "Q" ]; then
            echo "❌ User canceled."
            pause
            exit 1
        fi

        # Basic validation: should look like COM<number>
        if [[ "$ESP_DEVICE" =~ ^COM[0-9]+$ ]]; then
            echo "✅ Using $ESP_DEVICE"
            break
        else
            echo "⚠️ '$ESP_DEVICE' is not a valid COM format. Please try again (example: COM6)."
        fi
    done
}
